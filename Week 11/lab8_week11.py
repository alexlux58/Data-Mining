...
'''
There are 64 total features representing each pixel in an 8x8 square.
Their names are integers from [0,63) and each feature/pixel has an integer value from [0,16).
'''
...
for p in P:
  for c in C:
    for s in S:
      try:
        lr = LogisticRegression(penalty=p, C=c, solver=s, random_state=123)
...
lr_best = LogisticRegression(penalty='none', C=0.1, solver='sag', random_state=123)
...
print("The class_weight is the parameter that associates weights with classes.")
print("The default is that each class has weight 1.")
print("We don't need to modify that since our classes are balanced.")
...
best_pred = np.ndarray(shape=[0,0])
if k == "precomputed":
    kernel_train = np.dot(X_train_vanilla_svc, X_train_vanilla_svc.T) # linear kernel
...
print("Best LR classification report on test data:\n", creport(y_test, lr_pred, digits=4))
print("======================================= 2 ====================================================")
lin_svc_best = LinearSVC(penalty='l2', loss='hinge', C=0.1, random_state=123)
lin_svc_best.fit(X_train, y_train)
lin_svc_pred_best = lin_svc_best.predict(X_test)
print("Best LSVC classification report on test data:\n", creport(y_test, lin_svc_pred_best, digits=4))
print("======================================= 3 ====================================================")
svc_best = SVC(C=1, kernel='poly', degree=4, decision_function_shape='ovo', random_state=123)
svc_best.fit(X_train, y_train)
svc_pred_best = svc_best.predict(X_test)
print("Best SVC classification report on test data:\n", creport(y_test, svc_pred_best, digits=4))
print("==============================================================================================")
print("==============================================================================================")
print("The best model is SVC!")
print("Confusion matrix for SVC:\n", cmatrix(y_test, svc_pred_best))
print("The classes that get confused with each other are: 1 vs 6, 1 vs 8 (twice), 3 vs 9, and 4 vs 5")
print("It seems like the class '1' is most likely confused with other classes!")
...
scaler = MinMaxScaler()
X_train_norm_lr = scaler.fit_transform(X_train)
X_test_norm_lr = scaler.transform(X_test)

#hold-out validation. Do train_test_split on your training data:
X_train_new_lr, X_valid_lr, y_train_new_lr, y_valid_lr = train_test_split(X_train_norm_lr, y_train, test_size=0.3, random_state=123)
  ...
  for c in C:
    for s in S:
      try:
        ...
        lr.fit(X_train_new_lr, y_train_new_lr)
        pred = lr.predict(X_valid_lr)
        results[(p, c, s)] = f1_score(y_valid_lr, pred, average='weighted')
      except:
        continue
...
#You can use the hold-out dataset (the smaller subset) for the training, OR the bigger subset for training. Up to you!
lr_best = LogisticRegression(penalty='none', C=1, solver='sag')
lr_best.fit(X_train_new_lr, y_train_new_lr)
pred_best = lr_best.predict(X_valid_lr)
print(classification_report(y_valid_lr, pred_best, digits=4))
...
#SVC
scaler = MinMaxScaler()
X_train_norm_svc = scaler.fit_transform(X_train)
X_test_norm_svc = scaler.transform(X_test)
...
X_train_vanilla_svc, X_test_vanilla_svc, y_train_vanilla_svc, y_test_vanilla_svc = train_test_split(X_train_norm_svc, y_train, test_size=0.3, random_state=123)
...
lr_best = LogisticRegression(penalty='none', C=0.1, solver='sag', random_state=123)
lr_best.fit(X_train_new_lr, y_train_new_lr)
lr_pred = lr_best.predict(X_test_norm_lr)
print("Best LR classification report on test data:\n", creport(y_test, lr_pred, digits=4))
print("======================================= 2 ====================================================")
lin_svc_best = LinearSVC(penalty='l2', loss='hinge', C=0.1, random_state=123)
lin_svc_best.fit(X_train_svc, y_train_svc)
lin_svc_pred_best = lin_svc_best.predict(ntest)
print("Best LSVC classification report on test data:\n", creport(y_test, lin_svc_pred_best, digits=4))
print("======================================= 3 ====================================================")
svc_best = SVC(C=1, kernel='poly', degree=4, decision_function_shape='ovo', random_state=123)
svc_best.fit(X_train_norm_svc, y_train)
svc_pred_best = svc_best.predict(X_test_norm_svc)
print("Best SVC classification report on test data:\n", creport(y_test, svc_pred_best, digits=4))
print("==============================================================================================")
print("==============================================================================================\n")
print("The best model is SVC!")
